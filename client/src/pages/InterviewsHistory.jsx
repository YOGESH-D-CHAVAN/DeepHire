...
const InterviewsHistory = () => {
  ...
  return (
    <DashboardLayout>
      <div className="flex-1 p-4 md:p-8 pt-8 overflow-y-auto w-full max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold tracking-tight mb-1 flex items-center gap-3">
              <History className="w-6 h-6 md:w-8 md:h-8 text-cyan-400" />
              Interview History
            </h1>
            <p className="text-white/40 text-sm md:text-base">Review your past performance and track your growth.</p>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="grid gap-4">
            {filteredInterviews.map((interview, index) => (
              <motion.div
                ...
                key={interview._id}
                className="p-4 md:p-6 rounded-3xl bg-white/5 border border-white/10 hover:bg-white/[0.08] transition-all group"
              >
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 md:gap-6">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-white/40 text-xs md:text-sm line-clamp-1 max-w-xl">
                  ...
                  <div className="flex items-center justify-between lg:justify-end gap-8">
                    <div className="text-right">
                      <p className="text-[10px] md:text-xs font-bold text-white/40 uppercase tracking-wider mb-1">Score</p>
                      <div className="flex items-center gap-2">
                        <Award className="w-3 h-3 md:w-4 md:h-4 text-emerald-400" />
                        <span className="text-xl md:text-2xl font-black text-white">{interview.analysis?.score || 0}%</span>
                      </div>
                    </div>
                    
                    <Link 
                      to={`/analysis/${interview._id}`}
                      className="p-3 md:p-4 rounded-2xl bg-white text-black hover:bg-cyan-400 transition-colors"
                    >
          ...
          <div className="h-[50vh] flex flex-col items-center justify-center text-center p-6 md:p-8 bg-white/5 border border-white/10 rounded-[30px] md:rounded-[40px] border-dashed">
            <div className="w-16 h-16 md:w-20 md:h-20 rounded-3xl bg-white/5 flex items-center justify-center mb-6">
              <History className="w-8 h-8 md:w-10 md:h-10 text-white/20" />
            </div>
            <h2 className="text-xl md:text-2xl font-bold mb-2">No Interviews Found</h2>
            ...
            <Link to="/interview" className="w-full md:w-auto px-8 py-4 bg-cyan-500 text-[#0a0a0a] font-bold rounded-2xl hover:bg-cyan-400 transition-colors">
